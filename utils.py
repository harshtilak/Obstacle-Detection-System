...
#The code for generating random colours for the bounding boxes has been taken from: 
#https://stackoverflow.com/questions/28999287/generate-random-colors-rgb
#The code that has been taken and adapted begins here.
number_of_colors = 30

#For generating random colours for the bounding boxes.
random.seed(1)

color = ["#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])
             for i in range(number_of_colors)]
#The code that has been taken and adapted ends here.
...
def check_depth(img, det, lbls,depth_f,depth_y, conf=None, colours=COLOURS, class_map=weights.meta["categories"]):
    for i, ( tlx, tly, brx, bry) in enumerate(det):
        ...
        depth_f[i]=depth_data+(depth_data*(di*(bry+tly)/(36.8*(brx+tlx))))
        print( "check_depth car=> "+str(i)+" X Y => "+str(cx)+" "+str(cy)+" => "+ str(depth_data))

# annotate information to the bounding boxes
def annotate_class(image_name,img, det, lbls,depth_e,depth_x, conf=None, colours=COLOURS, class_map=weights.meta["categories"]):
    my_array = np.array([])
    for i, ( tlx, tly, brx, bry) in enumerate(det):
        txt = class_map[lbls[i]]
        if conf is not None:
        ...
        cx=math.floor((tlx+brx)/2)
        cy=math.floor((tly+bry)/2)
        depth_q=depth_x[tly:bry,tlx:brx].reshape(-1)
        depth_q=depth_q[depth_q>0]
        if(len(depth_q)>0):
            depth_q=np.median(depth_q)

            print("DATA =>"+image_name+ " => "+str(i)+",X,Y,stereo_depth,actual_depth => "+str(cx)+","+str(cy)+","+str(depth_e[i])+","+str(depth_q))
            if(not np.isnan(depth_e[i])):
                cv2.putText(img,str(math.floor(depth_e[i]))+'mt('+ str(i)+')', (brx-100, bry-12), fontFace=ff, fontScale=1, color=(120,0,0,255))
                my_array = np.append(my_array,  ([depth_e[i],depth_q]))
    return my_array
